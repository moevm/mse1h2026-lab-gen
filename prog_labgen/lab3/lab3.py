...
class Lab3Task(BaseTask):
    ...
    def render_assignment(self) -> str:
        ...
        lines = [
            ...
            "Программа должна считать текст из stdin до маркера ###, выделить предложения, обработать их и напечатать результат.",
            "Предложения заканчиваются одним из символов: . ; ? !",
            "Определения:",
            "- слово - это последовательность символов внутри предложения, отделённая от других слов пробелами или символами табуляции;",
            "- слова разделяются одним или несколькими символами пробела или табуляции;",
            "- знаки пунктуации и специальные символы считаются частью слова, если не отделены пробелами;",
            "- символы окончания предложения (. ; ? !) не входят в состав предложения и не считаются частью последнего слова;",
            "- максимальная длина слова ограничена параметром WordMax;",
            "Порядок обработки:",
            "- сначала выполняется select_rule;",
            "- затем для выбранных предложений выполняется rewrite_rule;",
            "- после rewrite_rule для каждого итогового предложения применяется keyword_rule;",
            "Общие требования:",
