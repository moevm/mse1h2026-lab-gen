...
def test_render_assignment_is_deterministic():
    t1 = Lab2Task(seed="ab12", Nmax=100, K=3)
    t2 = Lab2Task(seed="ab12", Nmax=100, K=3)
...
def test_render_assignment_contains_required_modules():
    task = Lab2Task(seed="ab12", Nmax=100, K=3)
...
def test_variant_contains_k_steps():
    task = Lab2Task(seed="ivanov", Nmax=50, K=2)
    variant = task._build_variant()
    assert len(variant.steps) == 2


def test_generate_tests_basic_shape():
    task = Lab2Task(seed="ab12", Nmax=50, K=2)
...
def test_every_test_has_exactly_k_output_lines():
    task = Lab2Task(seed="ab12", Nmax=50, K=3)
    tests = task.generate_tests()
    for test in tests:
...
@pytest.mark.parametrize("seed", ["ab12", "ivanov", "petrov"])
def test_variant_has_at_least_three_core_functions(seed: str):
    task = Lab2Task(seed=seed, Nmax=100, K=3)
    variant = task._build_variant()
    assert len(variant.core_functions) >= 3


def test_every_core_function_is_used_in_at_least_one_step():
    task = Lab2Task(seed="lalala", Nmax=100, K=4)
    variant = task._build_variant()
...
@pytest.mark.parametrize(
    ("kwargs", "error_part"),
    [
        ({"seed": "", "nmax": 50, "k": 2}, "--seed"),
        ({"seed": "ab12", "nmax": 0, "k": 2}, "--n-max"),
        ({"seed": "ab12", "nmax": 50, "k": 0}, "--k"),
    ],
)
...
def test_check_from_text_blob_keeps_lab2_check_text_functionality(capsys):
    ...
    variant = Lab2Task(seed="ab12", nmax=100, k=3)._build_variant()
    blob = (
        f"###{variant.main_file}###\n"
        "int main(void) {\n"
        "    return 1;\n"
